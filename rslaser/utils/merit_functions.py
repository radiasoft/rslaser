# -*- coding: utf-8 -*-
"""Merit functions used for nonlinear optimization
...
def spline_wfs(x, y, xc, yc, r_mid, r_max, phi_min, phi_max, r0):
    ...
    phi_mid = phi_max * (1.0 - (r_mid / r0) ** 2)
    c0 = (phi_mid - phi_min) / (1.0 - r_mid / r_max)

    # evaluate the function
    f_1 = phi_max * (1.0 - rsq / r0**2)  # fcn value for r<r_mid
    f_2 = phi_min + c0 * (1.0 - r / r_max)  # fcn value for r>r_mid
...
def spline_wfs_fit(params, x, y, data, xc, yc, r_mid, r_max, phi_min):
    ...
    weighted_diff = np.where(r <= r_mid, (g - data.reshape(g.shape)) / r, 0.0)
...
def azimuthalAverage(image, centerx, centery, type="mean"):
    ...
    if type == "mean":
        aaa = [
            np.nanmean(i_sorted[rind3[i] : rind3[i + 1]]) for i in range(len(rind3) - 1)
        ]
    elif type == "median":
        aaa = [
            np.nanmedian(i_sorted[rind3[i] : rind3[i + 1]])
            for i in range(len(rind3) - 1)
        ]
    elif type == "mode":
        ...
        for part in aaa_list:
            ...
            if len(xed) != 2:
                aaa.append(
                    0.5 * (xed[1:] + xed[:-1])[np.where(counts == np.max(counts))[0]][0]
                )
            elif len(xed) == 2:
                aaa.append(
                    0.5 * (xed[1:] + xed[:-1])[np.where(counts == np.max(counts))[0]][0]
    ...
    aaa_std = [
        np.nanstd(i_sorted[rind3[i] : rind3[i + 1]]) for i in range(len(rind3) - 1)
    ]
