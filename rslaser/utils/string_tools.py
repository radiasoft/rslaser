# -*- coding: utf-8 -*-
"""Convenient string manipulation routines
...
def wordwrap(line, lineWidth, endLine="", indenting=0):
    ...
    while lineBegin + lineWidth < len(line):
        ...
        while (
            line[lineEdit] not in string.whitespace or insideQuote(line, lineEdit)
        ) and lineEdit > lineBegin:
...
def stripComments(line, commentCharacter):
    for i in range(len(line)):
        if (
            line[i] == commentCharacter
            and not insideQuote(line, i)
            and not characterEscaped(line, i)
        ):
            return line[:i].strip()
    return line.strip()
